	...
	<table class="sortable" id="sortable_example" cellspacing="0" cellpadding="3" border="0" width="100%">
		...
		<tr class="ticketHeading"><th class="startsort">Date</th><th>Venue</th><th>Event</th><th class="unsortable">&nbsp;</th></tr>
       	 	...
       	 	{section name=production loop=$Productions}
        	        <tr><td>{$Productions[production].date}</td><td>{$Productions[production].venuename}</td><td>{$Productions[production].eventDescr}</td><td><b><a href="{$Productions[production].url}"><img src="{$RootUrl}/Images/tickets_vb.gif" alt="{$EventName} Tickets at {$Productions[production].venuename_no_loc} in {$Productions[production].city}, {$Productions[production].state} on {$Productions[production].ticket_page_title_date}" /></a></b></td></tr>
       	 	{/section}
