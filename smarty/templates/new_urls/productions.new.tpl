...
<div class="production_table">
	{if $DisplayHomeAwayOption > 0}
		<form name="input" action="{$ScriptName}" method="get">
			{if $HomeOnlyFlag == 1}
			<input id="First" type="radio" name="home_only" value="2" onclick="this.form.submit()"/>Next 20 Games
			<input id="Home" type="radio" name="home_only" value="1" checked="checked" onclick="this.form.submit()"/>Home Games Only
			...
			<input id="Away" type="radio" name="home_only" value="0" checked="checked" onclick="this.form.submit()"/>Home and Away Games
			{elseif $HomeOnlyFlag == 2}
			<input id="First" type="radio" name="home_only" value="2" checked onclick="this.form.submit()"/>Next 20 Games
	...
	<table class="sortable" id="sortable_example" cellspacing="0" cellpadding="3" border="0" width="100%">
		...
		<tr class="ticketHeading"><th class="startsort">Date</th><th>Venue</th><th>Event</th><th class="unsortable">&nbsp;</th></tr>
       	 	...
       	 	{section name=production loop=$Productions}
        	        <tr><td>{$Productions[production].date}</td><td>{$Productions[production].venuename}</td><td>{$Productions[production].eventDescr}</td><td><b><a href="{$Productions[production].url}"><img src="{$RootUrl}/Images/tickets_vb.gif" /></a></b></td></tr>
       	 	{/section}
