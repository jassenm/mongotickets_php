...
if(isset($_REQUEST['id']) && ($_REQUEST['id'] < 100000000) && ($_REQUEST['id'] >= 0) ) {
	$id = $_REQUEST['id'];
...
else
{
	# 5xx status code
	 ...
	 echo 'Sorry, there was an internal error. Please try again later.';
...
include('../include/host_info.inc.php');
	...
	# foreach($sorted_array as $index=>$ticket_info) {
                echo '<form action="/ticket_order.new_urls.php" method="post">';
                echo '<table width="100%" cellspacing="0">';
