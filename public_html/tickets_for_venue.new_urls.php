...
if((isset($_REQUEST['san_city'])) && (isset($_REQUEST['event_id']) && ($_REQUEST['event_id'] < 100000) && ($_REQUEST['event_id'] >= 0))) {
	...
	handle_error_no_exit ('tickets_for_venue.new_urls.php: neither san_city nor event_id or valid event_id provided ' .
...
if( $dbh=mysql_connect ($host_name, $db_username, $db_password) ) {
	...
	$Bsql = "SELECT Productions.EventID, VenueName, DATE_FORMAT(EventDate, '%a. %M %e, %Y %h:%i %p'), City, RegionCode, Venues.VenueID, ProductionID, EventName, Events.CategoryID, Events.EventTypeID, DATE_FORMAT(EventDate, '%M %e, %Y')  FROM Productions LEFT JOIN (Venues)  ON (Venues.VenueID = Productions.VenueID) LEFT JOIN (Events) ON (Events.EventID = Productions.EventID) where Productions.EventID= " . $eventID . " AND Venues.SanitizedCity = '" . $sanCity . "' AND DATEDIFF(NOW(), EventDate) <= 0 ORDER BY EventDate";
	if($query_result = mysql_query($Bsql) ) {
		...
		if($num_rows > 0) {
			while ($table_row = mysql_fetch_row($query_result)) {
				...
				$productions[] = array("date" => $eventDate, "venuename" => $venueNameLocation, "eventDescr" => "$eventName", "url" => "$url", "city" => "$city", "state" => "$regionCode", "ticket_page_title_date" => "$ticket_page_title_date", "venuename_no_loc" => "$venueName");
			}
		}
		else {
