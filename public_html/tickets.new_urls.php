...
if(isset($_REQUEST['production_id']) && ($_REQUEST['production_id'] >= 0) && isset($_REQUEST['event_id']) ) {
...
#print_r($getSearchTicketsResult);
	
#	echo '<br>$serverpath =' .$serverpath;
#	echo '<br><br>$soapclient = new SoapClient($serverpath, array("trace" => 1));';
	$soapOptions =array("trace" => 1, "encoding"=>'ISO-8859-1');
# Jassen 6/29 change since wasn't working	$soapclient = new SoapClient($serverpath, $soapOptions);
...
#	echo '<br>$param = array(  "SecurityToken" => "$securitytoken",  "ProductionID" => "$id", "MaximumPrice" => "1000000");';
	$param = array(  "SecurityToken" => "$securitytoken",  "ProductionID" => "$id");
	#$param = array(  "SecurityToken" => "$securitytoken",  "ProductionID" => "$id", "MaximumPrice" => 1000000);
#	echo '<br><br>print_r($param);';
#	echo '<br>';
#	print_r($param);
...
#	echo '<br>try<br> <br>';
	try
	{
		#echo '<br>$result = $soapclient->SearchTickets($param);';
		$result = $soapclient->SearchTickets($param);
		#	
 		#echo "<br>" . htmlspecialchars($soapclient->debug_str, ENT_QUOTES) . "\nEND_DEBUG_STR\n";
		#
#		print "<pre>\n";  
#		print "?????? :\n".htmlspecialchars($soapclient->__getLastRequest())."\n";  
#		echo '<br><br>$soapclient->__getLastResponseHeaders():<br>';
 #   		print($soapclient->__getLastResponseHeaders());
#		echo '<br><br>$soapclient->__getLastResponse():<br>';
#		print htmlspecialchars($soapclient->__getLastResponse(), ENT_QUOTES);
#		$parser = xml_parser_create(''); 
 #if(!$parser)
  #      return false;
  #  xml_parse_into_struct($parser, trim($soapclient->__getLastResponse()), $xml_values);
  #  xml_parser_free($parser); 
  #  echo "<br> dumping xml parser result <br>";
  #  var_dump($xml_values[11]);
  #  var_dump($xml_values[12]);
  #  var_dump($xml_values[13]);
		
#		echo "<br>";  
#		print "</pre>"; 
#		print "<br>here2<br><br>".htmlspecialchars($soapclient->__getLastRequestHeaders())."\n";  
#		print "<br><br>".htmlspecialchars($soapclient->__getLastRequest())."\n";  
...
#		$getSearchTicketsResult = $result->SearchTicketsResult;
#		echo '<br>var_dump($result); :<br>';
#		var_dump($result);
#		echo '<br><br>$result->SearchTicketsResult<br>';

		
#libxml_use_internal_errors(true);
#echo "<br><br>Simple XML <br>";
#$soap_root = simplexml_load_string(trim($soapclient->__getLastResponse()));
#$xml_data = get_object_vars($soap_root);#
#
#if (!$xml_data) {
...
#		echo "<br><br>Simple XML colon replace <br>";
		$tickets = array();
		try {
			libxml_use_internal_errors(true);
			$soap_root = new SimpleXMLElement($xmlString);
	
			#handle_error_no_exit ("tickets.code: ". $error->message);
			$data= $soap_root->soapBody->SearchTicketsResponse->SearchTicketsResult->ROOT->DATA;
 
			foreach ($data->row as $ticket)
			{
				$attrib = $ticket->attributes();
				$tprice = $attrib->TicketPrice;
				$tid = $attrib->TicketID;
				$bprice = $attrib->BrokerPrice;
				$bid = $attrib->BrokerID;
				$avail = $attrib->Available;
				$edate = $attrib->EventDate;
				$sec = htmlspecialchars($attrib->SeatSection);
				$seatdescr = mysql_escape_string($attrib->SeatDescription);
				$sdescr = htmlspecialchars($attrib->SeatDescription);
				$srow = htmlspecialchars($attrib->SeatRow);
				$sfrom = $attrib->SeatFrom;
				$sthru = $attrib->SeatThru;
	
	
/*
		[1] => SimpleXMLElement Object ( 
			[@attributes] => Array ( 
				[TicketID] => 39369889 [Available] => 6 [EventDate] => 2010-01-18T11:00:00 [SeatSection] => Upper Level [SeatRow] => N/A [SeatFrom] => [SeatThru] => [SeatDescription] => [TicketPrice] => 198 [BrokerPrice] => 0 [BrokerID] => 0 
			) 
		) 
	) 
) */
...
#}
				...
				#print_r($tickets);
			} #end for each ticket grp
		} # end try for SimpleXML parsing
#	handle_error_no_exit ("tickets.code: No result from web services");
	...
	{
		handle_error_no_exit ("tickets.new_urls.php: ". $flt);
...
if(($id == 557143) || ($id == 557144) || ($id == 557145)) {
	$subheading .= "<br />Crosstown Classic Tickets";
}
...
#########
# OLD, non-framed
 echo '<td width="' . $BUYNOW_WIDTH . '"><a href="https://www.ticketsnow.com/buytickets.aspx?id=' . $ticketid . '&amp;reqqty=0&amp;client=4089" rel="nofollow"><img src="' . $root_url . '/Images/tickets_br.gif" onmouseover="this.src=\'' . $root_url . '/Images/tickets_br.gif\';" onmouseout="this.src=\'' . $root_url . '/Images/tickets_br.gif\';" alt="' . "Buy $eventName $venueName Section " . $sorted_array[$j]['SeatSection'] . " Row " . $sorted_array[$j]['SeatRow'] . ' tickets"/></a></td>';
