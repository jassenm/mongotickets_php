...
include('../../include/host_info.inc.php');
       ...
       elseif($home_only == 2) { 
                $bsql = "SELECT ProductionID, DATE_FORMAT(EventDate, '%a. %M %e, %Y %h:%i %p'), VenueName, e1.EventName, e1.EventTypeID, City, RegionCode, e2.EventID, e2.EventName, ShortNote FROM Productions left join Venues on (Venues.VenueID = Productions.VenueID) left join Events as e1 on (e1.EventID = Productions.EventID) left join Events as e2 on (e2.EventID = Productions.OpponentEventID) where Productions.EventID=" . $eventID . " OR Productions.OpponentEventID=" . $eventID . " ORDER BY EventDate ASC LIMIT $max_display";
       }
	$num_productions = 0;
	$count = 0;
       if($query_result = mysql_query($bsql)) {
