...
if(isset($_REQUEST['production_id']) && ($_REQUEST['production_id'] < 1000000) && ($_REQUEST['production_id'] >= 0) && isset($_REQUEST['event_id']) ) {
	...
	handle_error_no_exit ('tickets.php: event_id not provided or production_id is missing or invalid, event_id= ' . $_REQUEST['event_id'] . ' production_id=' . $_REQUEST['production_id']  . ' ' .  $_SERVER['REQUEST_URI'] . ' returning 301');
	redir_301();
}
...
if( $dbh=mysql_connect ($host_name, $db_username, $db_password) ) {
        ...
        $Bsql = "SELECT Productions.EventID, VenueName, DATE_FORMAT(EventDate, '%a. %M %e, %Y %h:%i %p'), City, RegionCode, Venues.VenueID, e1.EventName, e1.CategoryID, e1.EventTypeID, e2.EventName, ShortNote, DATE_FORMAT(EventDate, '%b %e, %Y'), SanitizedVenueName, e2.EventID FROM Productions LEFT JOIN Venues  ON (Venues.VenueID = Productions.VenueID) LEFT JOIN Events as e1 ON (Productions.EventID = e1.EventID) LEFT JOIN Events as e2 ON (Productions.OpponentEventID = e2.EventID) where ProductionID=$id";
...
# echo '<br/>' . "in event id = $inEventID, opp ev id = $secondaryEventID ev id = $eventID";
			handle_error_no_exit ('tickets.php: EventID=' . $eventID . ' and prodid = ' . $id . ' lookup returned 0 results: attempting to find appropriate redirect, uri= ' . $_SERVER['REQUEST_URI'] . ' ');
			...
			if($query_result = mysql_query('SELECT EventName FROM Events WHERE EventID = ' . $inEventID)) {
				...
				if(strlen($redirEventName) < 0) {
					handle_error_no_exit ('tickets.php: redirecting production id=' .  $id  .  ' eventid=' . $_REQUEST['event_id']  .  ' from uri= ' .  $_SERVER['REQUEST_URI']  . ' to home, returning 301');
					redir_301();
				}
				$url = make_event_url($redirEventName);
 				$url = ltrim  ( $url, '/');
				handle_error_no_exit ('tickets.php: redirecting production id=' .  $id  .  ' eventid=' . $_REQUEST['event_id']  .  ' from uri= ' .  $_SERVER['REQUEST_URI']  . ' to $url, returning 301');
				redir_301($url);
			}
			else {
				handle_error_no_exit ('tickets.php: query failed during redirect find: '  .  mysql_error() . ' production id' .  $id  .  ' eventid=' . $_REQUEST['event_id']  .  ' uri= ' .  $_SERVER['REQUEST_URI']  . ' to home returning 301');
				redir_301();
			}
		}
        }
...
#########
  		...
  		echo '<td width="18%"><input type="hidden" name="ticket_id" value="' . $ticketid . '"/>';
                ...
                echo '<input type="hidden" name="quantity" value="0"/> <input type="image" src="/Images/buy_tickets.gif" value="submit" alt="' . "Buy $eventName $venueName Section " . $sorted_array[$j]['SeatSection'] . " Row " . $sorted_array[$j]['SeatRow'] . ' tickets"></td>';
...
function print_page_nav($startFrom, $num_pages, $sortBy, $sortOrder) {
	...
	if($startFrom > 1) { 
		...
		echo '<a href="' . $url . "?sortBy=$sortBy&sortOrder=$sortOrder&startFrom=$prevStartFrom$query_string \" style=\"color: blue; \"><strong>&lt Previous</strong></a>&nbsp;&nbsp;"; 
	}
	for ($i=1; $i <= $num_pages; $i++) {
		...
		if($startFrom == $i) {
			...
			echo '<a href="' . $url . "?sortBy=$sortBy&sortOrder=$sortOrder&startFrom=$offset$query_string\" style=\"color: blue;\">&nbsp;<strong>$i</strong>&nbsp;</a>&nbsp;";
		}
	}
	$nextStartFrom = $startFrom+1;
	if($nextStartFrom <= $num_pages) {
		echo '<a href="' . $url . "?sortBy=$sortBy&sortOrder=$sortOrder&startFrom=$nextStartFrom$query_string \" style=\"color: blue; \"><strong> Next &gt;</strong></a>&nbsp;";
	}
}
