...
if($dbh=mysql_connect ($host_name, $db_username, $db_password)) {
	...
	if(is_array($categoryIDList)) {
		foreach ( $categoryIDList as $categoryIDArray) {
			$url = GetCategoryUrl($categoryIDArray['id']);
		...
		echo "Database error when getting all Events!";
	}


	$bsql = "SELECT EventName, Events.EventID,Events.CategoryID FROM `Events` INNER JOIN Productions ON Productions.EventID=Events.EventID WHERE CategoryID=1310 OR CategoryID=1111 GROUP BY Events.EventID";
        if($query_result = mysql_query($bsql) ) {
                while ($table_row = mysql_fetch_array($query_result)) {
                        $eventName = $table_row['EventName'];
                        $eventID = $table_row['EventID'];
			$categoryID = $table_row['CategoryID'];
                ...
                echo "Database error when getting all other events!";
        }

	$bsql = "SELECT SanitizedVenueName FROM `Venues` WHERE 1";
        if($query_result = mysql_query($bsql) ) {
                while ($table_row = mysql_fetch_array($query_result)) {
                        $sanVenueName = $table_row['SanitizedVenueName'];
                        $url= make_venue_url($sanVenueName);
                        $s->addItem($url, $dt, 'daily','0.4');
                }
        }
        else {
                echo "Database error when getting all venues!";
        }
