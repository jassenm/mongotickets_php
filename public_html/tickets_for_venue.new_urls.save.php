...
if( $dbh=mysql_connect ($host_name, $db_username, $db_password) ) {
	...
	$Bsql = "SELECT Productions.EventID, VenueName, DATE_FORMAT(EventDate, '%a. %M %e, %Y %h:%i %p'), City, RegionCode, Venues.VenueID, ProductionID, EventName, Events.CategoryID, Events.EventTypeID, DATE_FORMAT(EventDate, '%M %e, %Y')  FROM Productions LEFT JOIN (Venues)  ON (Venues.VenueID = Productions.VenueID) LEFT JOIN (Events) ON (Events.EventID = Productions.EventID) where Productions.EventID= " . $eventID . " AND Venues.SanitizedCity = '" . $sanCity . "' ORDER BY EventDate";
