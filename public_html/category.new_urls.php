...
$MIN_NFL_PRESEASON_SPECIAL_EVENTID = 16093;
$MAX_NFL_PRESEASON_SPECIAL_EVENTID = 16124;
				...
				if(($i != 0) && (($i % 2) == 0)) {
					...
					if($query_result = mysql_query('SELECT EventID,EventName FROM Events WHERE CategoryID = ' . $subcategories[$i]['id'] . ' ORDER BY EventName ASC')) {
						while ($table_row = mysql_fetch_array($query_result)) {
							...
							if(($eventID >=	$MIN_NFL_PRESEASON_SPECIAL_EVENTID) && 
								($eventID <= $MAX_NFL_PRESEASON_SPECIAL_EVENTID)) {
								#skip
							}
							else {
