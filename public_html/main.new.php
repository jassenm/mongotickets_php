...
$title = "Mongo Tickets - New England Patriots Tickets, High School Musical Tickets, NFL Football Tickets";
...
$keywords = "concert ticket, theater tickets, sports tickets, mlb tickets, nba tickets, nfl tickets, football tickets";
	...
	for ($i=0; $i < count($topThreeArray); $i++) {
		...
		elseif(count($subcategoryArray) > 0 ) {
			...
			foreach($subcategoryArray as $subcategory) {
       				$url= make_category_url($subcategory['name'], $subcategory['id']);
